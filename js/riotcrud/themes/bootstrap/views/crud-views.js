...
riot.tag2('panel', '<yield></yield> <button each="{color in colors}" onclick="{click}">Call {color}</button> <div show="{called}">I hear you {caller} </div>', 'panel_a{ display:block; width:400px; height:80px; background:red; border:solid; margin-bottom:20px; padding:5px; } panel_b{ display:block; width:400px; height:80px; background:lightblue; border:solid; padding:5px; } panel_c{ display:block; width:140px; height:50px; background:yellow; border:solid; float:right; margin:5px; padding:5px; }', '', function(opts) {
