...
riot.tag2('crud-modal-dialog', '<div id="modal-dialog" class="modal fade bs-example-modal-{opts.size || \'lg\'}" tabindex="-1" role="dialog" aria-hidden="true"> <div class="modal-dialog modal-{opts.size || \'lg\'}"> <div class="modal-content"> <div class="modal-header"> <button type="button" class="close waves-effect" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button> <h4 class="modal-title" id="myModalLabel2">{opts.title}</h4> </div> <div class="modal-body"> <yield from="body"></yield> </div> <div class="modal-footer"> <yield from="footer"></yield> </div> </div> </div> </div>', '#modal-dialog .card{ box-shadow: none; } #modal-dialog .card .body { font-size: 14px; color: #555; padding: 0; }', '', function(opts) {
...
riot.tag2('crud-header-dropdown', '<modal-delete-confirmation></modal-delete-confirmation> <crud-modal-dialog title="Upload {opts.title}" service="{opts.service}" trigger="{opts.service}_upload_modal" trigger-submit="fg"> <yield to="body"> <crud-upload service="{opts.service}" title="{opts.name}"></crud-upload> </yield> <yield to="footer"> <button type="button" class="btn btn-default waves-effect" data-dismiss="modal" onclick="{abort}">Abbort</button> <button type="button" class="btn btn-info waves-effect" onclick="{triggerData}" data-trigger="product_add_items">Upload</button> </yield> </crud-modal-dialog> <ul class="header-dropdown m-r--5"> <li class="dropdown"> <a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"> <i class="material-icons">more_vert</i> </a> <ul class="dropdown-menu pull-right"> <li each="{action in opts.actions}"> <a if="{action.active}" href="#" onclick="{actionClick}"> <i if="{action.name == \'create\'}" class="material-icons">add</i> <i if="{action.name == \'view\'}" class="material-icons">view_compact</i> <i if="{action.name == \'delete\'}" class="material-icons">remove</i> <i if="{action.name == \'edit\'}" class="material-icons">mode_edit</i> <i if="{action.name == \'save\'}" class="material-icons">save</i> <i if="{action.name == \'list\'}" class="material-icons">view_list</i> <i if="{action.name == \'print\'}" class="material-icons">local_printshop</i> <i if="{action.name == \'pdf\'}" class="material-icons">picture_as_pdf</i> <i if="{action.name == \'csv\'}" class="material-icons">insert_drive_file</i> <i if="{action.name == \'json\'}" class="material-icons">insert_drive_file</i> <hr style="margin:0;" if="{action.name == \'upload\'}"> <i if="{action.name == \'upload\'}" class="material-icons">file_upload</i> <span if="{action.active}" class="{action.count === 0 ? \'font-line-through font-italic\' : \'font-bold\'}"> {action.label} <small if="{action.count >= 0}">({action.count})</small> </span> </a> </li> </ul> </li> </ul>', '', '', function(opts) {
		var self = this;
...
riot.tag2('modal-delete-confirmation', '<div id="deleteConfirmation" class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-hidden="true"> <div class="modal-dialog modal-sm"> <div class="modal-content"> <div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span> </button> <h4 class="modal-title" id="myModalLabel2">Delete <i>{opts.model}</i></h4> </div> <div class="modal-body"> ID: {opts.id} <br> {opts.text} </div> <div class="modal-footer"> <button type="button" class="btn btn-default" data-dismiss="modal">Abbort</button> <button type="button" class="btn btn-danger" onclick="{confirm}">Delete</button> </div> </div> </div> </div> <link href="/bower_components/adminbsb-materialdesign/plugins/sweetalert/sweetalert.css" rel="stylesheet">', 'div.modal-backdrop.fade.in { z-index: 7!important; } div.modal-body { word-break: break-all; }', '', function(opts) {
        var self = this;
