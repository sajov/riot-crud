...
(function (window, riot, route) {
    ...
    var FeatherClientMixin = {
                ...
                this.on('*', function (event) {
                    switch (event) {
                        ...
                        case 'unmount':
                            ...
                            var map = Object.keys(_this2.eventMap);
                            for (var i = 0; i < map.length; i++) {
                                if (event == 'unmount') {
                                    RiotControl.off(_this2.eventMap[map[i]]);
                                } else {
                                    _this2.bindEvent(_this2.eventMap[map[i]], map[i]);
                                }
                            }
                            break;
                        default:
                            if (_this2.debug) console.info('FeatherClientMixin event:' + event, _this2.opts.title);
                            break;
                    }
                });
            } else {
        ...
        bindEvent: function bindEvent(event, fn) {
            var self = this;
        ...
        actionQuery: function actionQuery(id) {
            if (this.opts.view == 'list') return;
            this.loadData({ id: id });
        },
        ...
        actionCreateSave: function actionCreateSave() {
            ...
            self.service.create(data).then(function (result) {
        ...
        loadDependencies: function loadDependencies(cb) {
            ...
            if (self.dependencies && self.dependencies.length == 0) {
                ...
                RiotCrudController.loadDependencies(self.dependencies, self.opts.title, // TODO dynamic name
                self.loadSchema(cb));
            }
        },
        ...
        loadData: function loadData(query) {
            ...
            switch (self.opts.view) {
                case 'view':
                case 'edit':
                    if (query.id) self.service.get(query.id).then(function (result) {
                        self.data = result;
