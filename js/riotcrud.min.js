...
(function (window, riot, route) {
    ...
    var FeatherClientMixin = {
                ...
                this.on('*', function (event) {
                    ...
                    switch (event) {
                        ...
                        case 'unmount':
                            ...
                            break;
                        default:
                            console.info('FeatherClientMixin event:' + event, _this2.opts.title, _this2.test);
                            break;
                    }
                });
            } else {
        ...
        actionEditSave: function actionEditSave() {
            ...
            self.service.update(data[self.opts.idfield], data).then(function (result) {
                console.warn('save' + self.opts.service, result);
            }).catch(function (error) {
    ...
    var OptsMixin = {
        ...
        init: function init() {
            if (!this.mytest) this.mytest = this.opts.title;
            console.error('optsMixin ', this.mytest);
            this.on('*', function (event) {
                console.log(this.opts.title + ' ??? ' + event);
            });
        },
    ...
    var viewActionsMixin = {
        ...
        init: function init() {
            ...
            this.on('*', function (event) {
                ...
                if (event != 'before-mount' || event != 'update') var view = self.opts.view || 'undefined';

                self.opts.actions = actions.map(function (action, index) {
